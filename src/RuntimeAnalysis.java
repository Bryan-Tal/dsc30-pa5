...
/**
 * This is an Analysis of different sorting runtimes
 */
public class RuntimeAnalysis {
    ...
    public static void main(String[] args) {
        ...
        numData2 = NUM_DATA;
        for (int i = 0; i < NUM_TEST; i++) {
            ArrayList<Integer> data = randomNumbers(numData2, MIN, MAX);
        ...
        numData3 = NUM_DATA;
        for (int i = 0; i < NUM_TEST; i++) {
            ArrayList<Integer> data = randomNumbers(numData3, MIN, MAX);
            timeQuickSort(data, NUM_RUN);
            numData3 += numData3;
        }
        int[] cutoff1 = {2,4,8,16,32,64,128};
        int size = 1000000;
        for (int i = 0; i < 7; i++) {
            ArrayList<Integer> data = randomNumbers(size, MIN, MAX);
            timeModifiedQuickSort(data, NUM_RUN, cutoff1[i]);
        }
        ...
        size2 = 100000;
        for (int i = 0; i < 6; i++) {
            ArrayList<Integer> data = randomNumbers(size2, MIN, MAX);
            timeModifiedQuickSort(data, NUM_RUN, cutoff2);
            size2 += 100000;
        }
        int size3;
        size3 = 100000;
        for (int i = 0; i < 6; i++) {
            ArrayList<Integer> data = randomNumbers(size3, MIN, MAX);
            timeQuickSort(data, NUM_RUN);
            size3 += 100000;
        }
    }
